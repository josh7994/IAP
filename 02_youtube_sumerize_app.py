...
def youtube_url_check(url):
    ...
    return match is not None # 일치하는 부분이 있으면 True, 없으면 False를 반환합니다.
...
def main():
    ...
    # session state 초기화
    # 요약 작업을 한 번만 실행하도록 제어하는 flag입니다. 초기값은 True.
    ...
    with st.sidebar:
        ...
        # 입력받은 API 키 값을 세션 상태에 저장
        if open_apikey:
            st.session_state["OPENAI_API"] = open_apikey 
        st.markdown('---')
    
    # 입력받은 URL이 유효한지 확인
    # URL이 입력되지 않았거나 길이가 2보다 작으면 에러 메시지
    if len(youtube_video_url)>2:
        if not youtube_url_check(youtube_video_url):
            ...
            #번역하기   
            transe = deepl_trans(st.session_state["summerize"])
            ...
            if st.button("대본 번역실행"):
                transe = deepl_trans(transcript[0])
                st.markdown(transe)
